...
public class BuncheolService {
  ...
  public void cancelBuncheol(final Long hostId, final Long buncheolId) {
    ...
    // 판정은 개최 목록 응답이 그대로 내려주는 것과 같은 값이다 (docs/56 S-2). 상태 위반(BLOCKED_BY_STATUS)은 여기서
    // 던지지 않고 CAS 에 맡긴다 — 경합에서 어느 상태로부터 전이됐는지는 CAS 만 알 수 있다.
    long confirmedCount =
        BuncheolHostCancellability.requiresConfirmedCount(buncheol.getStatus())
            ...
            : 0L;
    if (BuncheolHostCancellability.of(buncheol.getStatus(), confirmedCount)
        == BuncheolHostCancellability.BLOCKED_BY_CONFIRMED_PAYMENT) {
