...
/**
 * deadline 이 지난 RECRUITING 분철을 마감 판정(입금확인 인원 ≥ 최소 인원이면 진행확정, 미만이면 취소)한다. 마감시간은 도메인에서 정각으로
 * 강제되므로 매시 정각 cron 으로 정밀 마감(통지 지연 최소화)하고, fallback 폴링을 다운타임으로 cron 을 놓친 경우의 안전망으로 둔다(재기동 시
 * initial-delay 직후 1회로 빠르게 복구). 두 트리거 모두 멱등 + CAS 라 같은 분철을 중복 처리해도 안전하다. 분철별 독립
 * 트랜잭션({@link BuncheolAutoCloseService#finalizeExpired}) 이라 한 건 실패가 나머지를 막지 않는다.
 *
...
public class BuncheolAutoCloseScheduler {
  ...
  // 정시 정밀 마감: 마감시간이 도메인에서 정각으로 강제돼 매시 정각(KST)에 즉시 마감해 통지 지연을 최소화. 식은 app.buncheol.auto-close.cron 으로 조정.
  @Scheduled(cron = "${app.buncheol.auto-close.cron:0 0 * * * *}", zone = "Asia/Seoul")
  ...
  @Scheduled(
      fixedDelayString = "${app.buncheol.auto-close.fallback-interval-ms:600000}",
