...
public class KakaoOidcUserProfileExtractor implements OAuth2UserProfileExtractor {
  ...
  public OAuth2UserProfile extract(final OAuth2User principal) {
    ...
    // 카카오 ID 토큰에는 이름·전화번호·연령대 클레임이 기본 포함되지 않는다 — 값이 있으면 쓰고,
    // 없으면 성공 핸들러에서 KakaoApiClient(/v2/user/me)로 보강 조회한다 (연령대는 항상 보강 경로).
    ...
    return new OAuth2UserProfile(
        KAKAO, oidcUser.getSubject(), oidcUser.getEmail(), name, phoneNumber, null);
  }
