...
public class MyParticipationQueryService {
   ...
   *
   * <p>{@code null} 이 될 수 있는 경우는 셋이다 — 묶음이 없는 옛 행이고 사본도 비었거나, 가드 이전에
   * 배송지가 지워졌거나, <b>완주(배송 종료) 후 사용자가 그 배송지를 정리했거나(2026-09-08부터 정상 경로다
   * — 여기에 경고 로그를 달지 말 것, 완주 건마다 울린다)</b>. 어느 쪽도 500 을 낼 일은 아니다(목록 전체가 안 뜬다). 화면이
