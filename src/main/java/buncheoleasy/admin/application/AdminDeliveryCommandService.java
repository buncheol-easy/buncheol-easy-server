...
/**
 * 관리자 운송장 등록 벌크 처리. 배송 건별 독립 트랜잭션({@link DeliveryService#registerTrackingByAdmin})으로 등록하고 비즈니스
 * 실패는 사유와 함께 모아 돌려준다 — 기존 프론트가 배송 건마다 순차 호출하던 것을 단일 요청으로 대체한다.
 *
...
public class AdminDeliveryCommandService {
  ...
  public AdminBulkResultResponse registerTracking(
    ...
    // 같은 id 중복 요청은 순서를 유지한 채 제거한다 (등록 자체는 멱등이지만 응답에 이중 집계되는 것을 막는다).
    for (final Long deliveryId : new LinkedHashSet<>(deliveryIds)) {
      try {
        deliveryService.registerTrackingByAdmin(deliveryId, trackingNumber);
        ...
        log.error("운송장 벌크 등록 중 예상 밖 실패. deliveryId={}", deliveryId, exception);
        failures.add(
            new AdminBulkResultResponse.Failure(
                deliveryId,
