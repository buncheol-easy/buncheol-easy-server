 ...
 *
 * <ul>
 *   <li>그룹 순위: 지점명 일치 = {@value #RANK_NAME}, 주소만 일치 = {@value #RANK_ADDRESS}. "강남" 검색 시
 *       강남 지점명 매장이 강남대로 소재 매장보다 먼저 노출되게 한다. 키워드가 없으면 전체가 {@value #RANK_NAME} 그룹이다.
 ...
 *
 * <p>그룹 판정 기준(지점명의 키워드 포함 여부)은 마스터 데이터라 페이지네이션 도중 바뀌지 않으므로,
 * {@link buncheoleasy.buncheol.domain.BuncheolListCursor} 와 달리 그룹 간 중복 노출이 없다.
 */
public record CvsStoreCursor(Integer groupRank, Long id) {
  ...
  /**
   * 직전 페이지의 마지막 접수처로부터 다음 커서를 만든다. 그룹 판정은 지점명의 키워드 포함 여부(대소문자 무시 —
   * MySQL {@code utf8mb4_unicode_ci} 매칭과 정합)로 하며, {@code trimmedKeyword} 는 이스케이프 전의 원문이어야 한다.
   */
